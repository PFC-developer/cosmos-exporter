	...
	if Oracle {
		for _, val := range s.Validators {
			...
			if err != nil {
				...
				doPryzmMetrics(&wg, &sublogger, pryzmMetrics, s, s.Config, valAddress)
			}
		}
	}
	if len(s.Wallets) > 0 {
		...
		go func() {
			...
			if s.Config.PropV1 {
				...
				if err != nil {
					sublogger.Error().
						Err(err).
						Msg("Could not get active proposals V1 (pryzm)")
				}
			} else {
				...
				if err != nil {
					sublogger.Error().
						Err(err).
						Msg("Could not get active proposals (pryzm)")
				}
			}
		}()
